...
var rangeSum = function (nums, n, left, right) {
  // Taking subarrays sum, putting in an array, sort it, then getting sum from left to right and returning it
  let res = [];
  for (let i = 0; i < n; i++) {
    // pushing numbers itself as they also are individual subarrays
    res.push(nums[i]);
  }
  ...
  for (let i = left - 1; i < right; i++) {
    // As left and right are 1-based indexed so we do left-1 and <right
    ans += res[i];
  }

  // % (1e9 + 7) is given so we have to do it
  return ans % (1e9 + 7);
};
