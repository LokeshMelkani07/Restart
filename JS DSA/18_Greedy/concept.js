...
var bagOfTokensScore = function (tokens, power) {
  // We need to think in a greedy manner
  // When we are utilising power, spend it on as low token as possible
  // When we are utilising our score, spend it in as highest power as possible
  // We can first sort token array to get lowest and highest power tokens in order
  // We will keep 2 pointers i = 0 and j = n-1 to point to highest and lowest power token
  let score = 0;
  let i = 0,
    j = tokens.length - 1;
  let maxScore = 0;
  ...
  while (i <= j) {
    if (power >= tokens[i]) {
      // if we can utilise our power, utilise from start of array and update maxScore
      power = power - tokens[i++];
      maxScore = Math.max(maxScore, ++score);
    } else if (score >= 1) {
      // if we have score, utilise it and add maximum power to power from end of array
      power = power + tokens[j--];
      maxScore = Math.max(maxScore, --score);
    } else {
      // if neither we have power to spend nor we have score then break
      break;
    }
  }
